...
final class ProfileViewController : UIViewController {
    ...
    private lazy var descriptionLabel: UILabel = {
       let label = UILabel()
        label.text = "님은 바로!! 사회주의 낙원을 말하는 것입니다, 여러분! (박수갈채)"
    ...
    private let photoDataView = ProfileDataView(title: "게시물", count: 1972)
    private let follwerDataView = ProfileDataView(title: "팔로워", count: 11)
    private let followingDataView = ProfileDataView(title: "팔로잉", count: 21)
...
private extension ProfileViewController {
    func setupNaivgationItems() {
        navigationItem.title = "심 영"
        
        let rightBatButton = UIBarButtonItem(image: UIImage(systemName: "ellipsis"), style: .plain, target: self, action: #selector(didTapRightBarButtonItem))
    ...
    func setupLayout () {
        ...
        let dataStackView = UIStackView(arrangedSubviews: [
            photoDataView, follwerDataView, followingDataView
        ])
